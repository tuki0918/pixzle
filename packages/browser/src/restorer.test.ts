...
vi.mock("@pixzle/core", async () => {
  ...
  return {
    ...
    createSingleImageManifest: (options: {
      blockSize: number;
      seed: string;
    ...
    validateManifestVersion: (manifest: { version: string }) => {
      if (/^0\.[0-2]\./.test(manifest.version)) {
        throw new Error(
...
describe("ImageRestorer", () => {
  ...
  it("should reject manifests generated by v0.2.x or older before restoring", async () => {
    const restorer = new ImageRestorer();
    const mockImage = new MockImageBitmap(100, 100) as unknown as ImageBitmap;

    await expect(
      restorer.restoreImages([mockImage], {
        id: "legacy",
        version: "0.2.9",
        timestamp: new Date().toISOString(),
        config: {
    ...
    expect(blockModule.imageToImageBuffer).not.toHaveBeenCalled();
  });
});
