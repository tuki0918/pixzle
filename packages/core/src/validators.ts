...
export function validateManifestVersion(
  manifest: Pick<ManifestData, "version">,
): void {
  const version = parseVersion(manifest.version);
  const minVersion = parseVersion(MIN_RESTORABLE_MANIFEST_VERSION);

  if (!minVersion) {
    throw new Error(
      `Invalid minimum restorable manifest version: ${MIN_RESTORABLE_MANIFEST_VERSION}`,
    );
  }
